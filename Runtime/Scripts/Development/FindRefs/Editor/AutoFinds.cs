...
{ 
    ...
    { 
        public static void GetAutoRefs(Transform isSelf)
        {
            // Get all game objects.
            var acGameObjects = isSelf ? new GameObject[isSelf.childCount] : FindObjectsOfType<GameObject>();
            ...
            for (int nGameObject = 0; nGameObject < acGameObjects.Length; nGameObject++)
            {
                ...
                if(acMonoBehaviours == null) continue;
                ...
                for (int i = 0; i < acMonoBehaviours.Length; i++)
                {
                    ...
                    if(cMonoBehaviour == null) continue;
                    ...
                    for (int nFieldInfo = 0; nFieldInfo < acFieldInfos.Length; nFieldInfo++)
                    {
                        ...
                        if (Attribute.GetCustomAttribute(cFieldInfo, typeof(FindsAttribute)) is FindsAttribute cFinds)
                        {
                            // Get the targets to be searched.
                            var lstAutoRefTargets = GetAutoRefTargets(cGameObject, cFinds.findType, cFinds.name);
